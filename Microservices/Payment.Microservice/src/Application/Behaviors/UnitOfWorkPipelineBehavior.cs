    ...
    where TRequest : notnull
{
    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError(ex, "Command {Command} threw an exception. Changes were not saved.", typeof(TRequest).Name);
            throw;
        }
        ...
        {
            _logger.LogError(ex, "Command {Command} failed during persistence. Changes were not saved.", typeof(TRequest).Name);
            throw;
        }
